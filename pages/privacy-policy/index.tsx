...
const PrivacyPolicyPage: FC = ({ locale }: { locale: 'cn' | 'en' }) => {
  // language keys
  ...
  const policy3List4 = i18next.t("policy3List4");
  ...
  const policy10Text4 = i18next.t("policy10Text4");
  ...
  return (
    <LayoutPage className="pb-48 text-justify text-white bg-gradient-gray1">
      ...
      <div className="container m-auto bg-white rounded-2.5xl text-grayProfile shadow -mt-40 divide-x flex flex-col px-10">
        ...
        <section className="w-full text-right text-color1 py-4 underline">
          {downloadDocument}
          <a href={`/privacy-policy/${locale === 'cn' ? 'protection_policy_cn' : 'protection_policy_en'}.pdf`} target="_blank">{' '}{privacy}</a>
        </section>
      </div>
    </LayoutPage>
  );
