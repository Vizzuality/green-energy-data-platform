...
const PaginatedDynamicChart = ({
  ...
  const data = useMemo(
    () => sortedData.slice(pagination.from, pagination.to),
    [pagination.from, pagination.to, sortedData],
  );
        ...
        <Button onClick={() => handleClick('prev')}>
          <Icon
            ...
            className={classNames(
              'p-2 mr-3 transform rotate-90 border-2 rounded-full border-gray1 border-opacity-30 text-gray1',
              {
                'cursor-pointer hover:bg-gray1 hover:bg-opacity-90 hover:text-white':
                  !isFirst,
